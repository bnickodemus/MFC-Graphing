...
private:
	...
	**/
	void Draw(CDC* pDC, int num);
	//! Draw the axis label
	/**
	  @param pDC
		The DC used to draw the axis.
	**/
	void DrawLabel(CDC* pDC, int num);
	...
	**/
	void DrawTick(CDC* pDC, double dTickVal, int num);
	...
	**/
	void SetAxisSize(const CRect& ControlRect, const CRect& MarginRect, int num);
