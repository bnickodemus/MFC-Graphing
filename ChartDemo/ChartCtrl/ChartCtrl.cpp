...
CChartStandardAxis* CChartCtrl::CreateStandardAxis(EAxisPos axisPos, int num)
{
...
CChartStandardAxis* CChartCtrl::CreateSpecialAxis(int num)
{
	CChartStandardAxis* pAxis = new CChartStandardAxis();
	...
	// Before storing the new axis, we should delete the previous one if any
	if (m_pAxes[num])
		delete m_pAxes[num];
	m_pAxes[num] = pAxis;
...
void CChartCtrl::DrawChart(CDC* pDC, CRect ChartRect)
{
	...
	for (n=0;n<8;n++)
	{
		if (m_pAxes[n])
		{
			m_pAxes[n]->SetAxisSize(ChartRect,m_PlottingRect,n);
...
void CChartCtrl::OnMouseMove(UINT nFlags, CPoint point) 
{
	if (m_bRMouseDown && m_bPanEnabled)
	{
		if (point != m_PanAnchor)
		{
			...
			if (m_pAxes[4])
				m_pAxes[4]->PanAxis(m_PanAnchor.y,point.y);
			if (m_pAxes[5])
				m_pAxes[5]->PanAxis(m_PanAnchor.y,point.y);
			if (m_pAxes[6])
				m_pAxes[6]->PanAxis(m_PanAnchor.y,point.y);
			if (m_pAxes[7])
				m_pAxes[7]->PanAxis(m_PanAnchor.y,point.y);
...
void CChartCtrl::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	if (m_bZoomEnabled)
	{
				  ...
				  (m_rectZoomArea.top!=m_rectZoomArea.bottom))
		{
			...
			if (m_pAxes[4])
			{
				if (m_pAxes[4]->IsInverted())
				{
					MaxVal = m_pAxes[4]->ScreenToValue(m_rectZoomArea.bottom);
					MinVal = m_pAxes[4]->ScreenToValue(m_rectZoomArea.top);
				}
				else
				{
					MinVal = m_pAxes[4]->ScreenToValue(m_rectZoomArea.bottom);
					MaxVal = m_pAxes[4]->ScreenToValue(m_rectZoomArea.top);
				}
				m_pAxes[4]->SetZoomMinMax(MinVal,MaxVal);
			}
			if (m_pAxes[5])
			{
				if (m_pAxes[5]->IsInverted())
				{
					MaxVal = m_pAxes[5]->ScreenToValue(m_rectZoomArea.bottom);
					MinVal = m_pAxes[5]->ScreenToValue(m_rectZoomArea.top);
				}
				else
				{
					MinVal = m_pAxes[5]->ScreenToValue(m_rectZoomArea.bottom);
					MaxVal = m_pAxes[5]->ScreenToValue(m_rectZoomArea.top);
				}
				m_pAxes[5]->SetZoomMinMax(MinVal,MaxVal);
			}
			if (m_pAxes[6])
			{
				if (m_pAxes[6]->IsInverted())
				{
					MaxVal = m_pAxes[6]->ScreenToValue(m_rectZoomArea.bottom);
					MinVal = m_pAxes[6]->ScreenToValue(m_rectZoomArea.top);
				}
				else
				{
					MinVal = m_pAxes[6]->ScreenToValue(m_rectZoomArea.bottom);
					MaxVal = m_pAxes[6]->ScreenToValue(m_rectZoomArea.top);
				}
				m_pAxes[6]->SetZoomMinMax(MinVal,MaxVal);
			}
			if (m_pAxes[7])
			{
				if (m_pAxes[7]->IsInverted())
				{
					MaxVal = m_pAxes[7]->ScreenToValue(m_rectZoomArea.bottom);
					MinVal = m_pAxes[7]->ScreenToValue(m_rectZoomArea.top);
				}
				else
				{
					MinVal = m_pAxes[7]->ScreenToValue(m_rectZoomArea.bottom);
					MaxVal = m_pAxes[7]->ScreenToValue(m_rectZoomArea.top);
				}
				m_pAxes[7]->SetZoomMinMax(MinVal,MaxVal);
			}
