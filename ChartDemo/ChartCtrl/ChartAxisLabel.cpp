...
void CChartAxisLabel::Draw(CDC *pDC, int num)
{
	if (!m_bIsVisible)
		return;
	if (!pDC->GetSafeHdc())
	...
	if (num == 7 )
	{
		pDC->ExtTextOut(m_TextRect.left+100,m_TextRect.top,
			ETO_CLIPPED,NULL,m_strLabelText.c_str(),NULL);
	}
	else if (num == 6)
	{
		pDC->ExtTextOut(m_TextRect.left+50,m_TextRect.top,
			ETO_CLIPPED,NULL,m_strLabelText.c_str(),NULL);
	}
	else if (num == 5)
	{
	...
	{
		pDC->ExtTextOut(m_TextRect.left+6,m_TextRect.top,
			ETO_CLIPPED,NULL,m_strLabelText.c_str(),NULL);
	}
	else if (num == 2)
	{
		pDC->ExtTextOut(m_TextRect.left-4,m_TextRect.top, 
			ETO_CLIPPED,NULL,m_strLabelText.c_str(),NULL);
	}
	else if (num == 1) // bottom
	{
		pDC->ExtTextOut(m_TextRect.left-100,m_TextRect.top+6, 
			ETO_CLIPPED,NULL,m_strLabelText.c_str(),NULL);
	}
	else
	{		
		pDC->ExtTextOut(m_TextRect.left,m_TextRect.top + 6, //2
			ETO_CLIPPED,NULL,m_strLabelText.c_str(),NULL);
	}
