...
void CChartAxis::Draw(CDC *pDC, int num)
{
	if (!m_bIsVisible)
		return;
	...
	switch (num) {
		...
		{ //L2
			pDC->MoveTo(90,100);
			pDC->LineTo(90,340); //525
		}
		case 5 :
		{ //L3
			pDC->MoveTo(45,100);
			pDC->LineTo(45,340); //525
		}
		case 6 :
		{ //R2
			pDC->MoveTo(698,100);
			pDC->LineTo(698,340); //525
		}
		case 7 :
		{ //R3
			pDC->MoveTo(748,100);
			pDC->LineTo(748,340); //525
		}
	}
	...
	if (m_MaxValue == m_MinValue)
		DrawTick(pDC,m_MinValue,num);
	else
	{
		double TickValue = GetFirstTickValue();
		do 
		{
			DrawTick(pDC,TickValue,num);
	...
	else // vertical
	{
		...
		if (!m_bIsSecondary)
		{
			if (bTickOnAxis)
			{
				if (num == 4)
				{
					pDC->MoveTo(m_AxisRect.right-61,TickPos);
					pDC->LineTo(m_AxisRect.right-64,TickPos);
				}
				else if (num == 5)
				{
					pDC->MoveTo(m_AxisRect.right-106,TickPos);
					pDC->LineTo(m_AxisRect.right-109,TickPos);
				}
				else
				{
			...
			if (bLabelOnAxis)
			{
				if (num == 4)
				{
					pDC->ExtTextOut(m_AxisRect.left+LabelSize.cx+48,lLabelPos-TextSize.cy/2,
						ETO_CLIPPED|ETO_OPAQUE,NULL,strBuffer.c_str(),NULL);
				}
				else if (num == 5)
				{
					pDC->ExtTextOut(m_AxisRect.left+LabelSize.cx+2,lLabelPos-TextSize.cy/2,
						ETO_CLIPPED|ETO_OPAQUE,NULL,strBuffer.c_str(),NULL);
				}
				else 
				{
					pDC->ExtTextOut(m_AxisRect.left+LabelSize.cx+104,lLabelPos-TextSize.cy/2,
						ETO_CLIPPED|ETO_OPAQUE,NULL,strBuffer.c_str(),NULL);
				}
			}
		}
		else
		{
			if (bTickOnAxis)
			{
				if (num == 6)
				{
					pDC->MoveTo(m_AxisRect.left+51,TickPos);
					pDC->LineTo(m_AxisRect.left+54,TickPos);
				}
				else if (num == 7)
				{
					pDC->MoveTo(m_AxisRect.left+101,TickPos);
					pDC->LineTo(m_AxisRect.left+104,TickPos);
				}
				else
				{
			...
			if (bLabelOnAxis)
			{
				if (num == 6)
				{
					pDC->ExtTextOut(m_AxisRect.left+55,lLabelPos-TextSize.cy/2,
						ETO_CLIPPED|ETO_OPAQUE,NULL,strBuffer.c_str(),NULL);
				}
				else if (num == 7)
				{
					pDC->ExtTextOut(m_AxisRect.left+105,lLabelPos-TextSize.cy/2,
						ETO_CLIPPED|ETO_OPAQUE,NULL,strBuffer.c_str(),NULL);
				}
				else 
				{
...
void CChartAxis::DrawLabel(CDC* pDC, int num)
{
	// Draw the axis label.
	CSize LabelSize = m_pAxisLabel->GetSize(pDC);
	...
	if (num == 4)
		m_pAxisLabel->SetPosition(XPos+40,YPos,pDC);
	else if (!m_bIsSecondary) 
		m_pAxisLabel->SetPosition(XPos+100,YPos,pDC);
	else
		m_pAxisLabel->SetPosition(XPos-100,YPos,pDC);
	m_pAxisLabel->Draw(pDC, num);
}
							 ...
							 int num)
{
	if (num == 4 || num == 5 || num == 6 ) {
		//MessageBox(NULL,L"num is 4",NULL,MB_OK);
...
int CChartAxis::ClipMargin(CRect ControlRect, CRect& MarginRect,CDC* pDC)
{
	...
	if (m_bIsHorizontal)
	{
		...
		{
			Size += 4 + 52;		//Space above and under the text
	...
	else
	{
		...
		{
			Size += 7 + 101;		//Space before and after the text + Tick
